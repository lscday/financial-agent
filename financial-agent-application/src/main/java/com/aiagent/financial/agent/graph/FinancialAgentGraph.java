...
import com.aiagent.financial.domain.service.AdvisoryWorkflowPolicy;
import com.aiagent.financial.domain.service.SettlementPolicy;
...
public class FinancialAgentGraph {
    ...
    public FinancialAgentGraph(RetrievalService retrievalService,
                               ...
                               ChatModel chatModel,
                               AdvisoryWorkflowPolicy workflowPolicy,
        ...
        try {
            ...
            // 路由规则收敛到领域服务 AdvisoryWorkflowPolicy
            EdgeAction<AgentState> riskRouting = state -> {
                if (workflowPolicy.shouldAnalyzeDirectly(RiskLevelEnum.fromCode(state.getRiskLevel()))) {
            ...
            // 条件边：清算重试循环（最多 maxRetries 次）
            // 路由规则收敛到领域服务 AdvisoryWorkflowPolicy
            EdgeAction<AgentState> settlementRouting = state -> {
                AdvisoryWorkflowPolicy.SettlementRoute route =
                        workflowPolicy.decideSettlementRoute(state.isSettlementSuccess(),
                                state.getRetryCount(), maxRetries);
                return switch (route) {
                    case END -> {
                        log.info("清算成功 → 结束");
                        yield GraphDefinition.END;
                    }
                    case ANALYZE -> {
                        log.warn("{} 次重试后清算仍然失败 → 进入分析节点", maxRetries);
                        yield AgentNode.ANALYSIS.nodeName();
                    }
                    case RETRY -> {
                        log.info("清算第 {}/{} 次尝试失败 → 重试", state.getRetryCount(), maxRetries);
                        yield AgentNode.SETTLEMENT.nodeName();
                    }
                };
            };
            graph.addConditionalEdges(
